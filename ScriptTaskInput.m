...
po_ALCA = zeros(length(Alpha), noRealizations); 
su_ALCA = zeros(length(Alpha), noRealizations); 

po_ARJOA = zeros(length(Alpha), noRealizations); 
su_ARJOA = zeros(length(Alpha), noRealizations); 

po_ODSTCA = zeros(length(Alpha), noRealizations); 
su_ODSTCA = zeros(length(Alpha), noRealizations); 

po_IOJOA = zeros(length(Alpha), noRealizations); 
su_IOJOA = zeros(length(Alpha), noRealizations); 

po_OFDMA = zeros(length(Alpha), noRealizations);
su_OFDMA = zeros(length(Alpha), noRealizations);

xt = cell(1, length(Alpha)); 

for iN = 1:length(Alpha)
	alpha = Alpha(iN); 
	xt(iN) = {num2str(alpha)};
    alpha = 1e6*alpha; 
    ...
    %%%%%%%%%%%%%%%%%%%%%
        ...
        % all users independenidsy make offloading decision 
        for i = 1:NoUsers
        	if rand() > 0.5
        		continue
        	end
...
save('datasize.mat', 'po_ODSTCA', 'po_ARJOA', 'po_IOJOA', 'po_ALCA', 'po_OFDMA', ...
'su_ODSTCA', 'su_ARJOA', 'su_IOJOA', 'su_ALCA','su_OFDMA')

Alpha = 0.2:0.1:1
xt = {'0.2', '0.4', '0.6', '0.8', '1'}
h2 = figure(2)
hold on 
plot(1:length(Alpha), po_ODSTCA(1:length(po_ODSTCA)), 'b-o', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), po_ARJOA(1:length(po_ARJOA)), 'g-v', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), po_IOJOA(1:length(po_IOJOA)), 'k-x', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), po_OFDMA(1:length(po_OFDMA)), 'r-s', 'linewidth', 2.0, 'markers', 13.0);
%plot(1:length(Alpha), po_ALCA(1:length(po_ALCA)), 'k-d', 'linewidth', 2.0, 'markers', 13.0);
grid on
box on
set(gca, 'FontSize', 20, 'XLim', [1 length(Alpha)]);
xticks = 1:2:length(Alpha);
set(gca, 'xtick', xticks);
set(gca, 'xticklabel',xt)
xlabel('Input Data Size (MB)');
...
hold
savefig(h2, 'ids_po.fig')

h3 = figure(3)
hold on 
plot(1:length(Alpha), su_ODSTCA(1:length(su_ODSTCA)), 'b-o', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), su_ARJOA(1:length(su_ARJOA)), 'g-v', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), su_IOJOA(1:length(su_IOJOA)), 'k-x', 'linewidth', 2.0, 'markers', 13.0);
plot(1:length(Alpha), su_OFDMA(1:length(su_OFDMA)), 'r-s', 'linewidth', 2.0, 'markers', 13.0);
%plot(1:length(Alpha), su_ALCA(1:length(su_ALCA)), 'k-d', 'linewidth', 2.0, 'markers', 13.0);

grid on
box on
set(gca, 'FontSize', 17.5, 'XLim', [1 length(Alpha)], 'YLim', [1.75 inf]);
xticks = 1:2:length(Alpha);
set(gca, 'xtick', xticks);
set(gca, 'xticklabel',xt);
xlabel('Input Data Size (MB)');
